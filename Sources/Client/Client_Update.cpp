...
DEFINE_SPADES_SETTING(cg_localDryFireSoundGain, "1");
...
#pragma mark - World Actions
		...
		void Client::SetSelectedTool(Player::ToolType type, bool quiet) {
			...
			if (!quiet) {
				...
				AudioParam param;
				param.volume = cg_switchToolSoundGain;
...
#pragma mark - IWorldListener Handlers
		...
		void Client::PlayerDryFiredWeapon(spades::client::Player &p) {
			...
			if (!IsMuted()) {
				...
				if (isLocal) {
					AudioParam param;
					param.volume = cg_localDryFireSoundGain;
					audioDevice->PlayLocal(c.GetPointerOrNull(), MakeVector3(.4f, -.3f, .5f), param);
				}
				else
		...
		void Client::PlayerRestocked(spades::client::Player &p) {
			if (!IsMuted()) {
				...
				if (isLocal) {
					AudioParam param;
					param.volume = cg_restockSoundGain;
					audioDevice->PlayLocal(c.GetPointerOrNull(), MakeVector3(.4f, -.3f, .5f), param);
				}
				else
					audioDevice->Play(c.GetPointerOrNull(),
		...
		void Client::PlayerThrewGrenade(spades::client::Player &p,
			...
			if (!IsMuted()) {
				...
				if (isLocal) {
					AudioParam param;
					param.volume = cg_throwNadeSoundGain;
					audioDevice->PlayLocal(c.GetPointerOrNull(), MakeVector3(.4f, 0.1f, .3f), param);
				}
				else
					audioDevice->Play(c.GetPointerOrNull(),
		...
		void Client::PlayerMissedSpade(spades::client::Player &p) {
			...
			if (!IsMuted()) {
				...
				if (isLocal) {
					AudioParam param;
					param.volume = cg_spadeMissSoundGain;
					audioDevice->PlayLocal(c.GetPointerOrNull(), MakeVector3(.2f, -.1f, 0.7f), param);
				}
				else
					audioDevice->Play(c.GetPointerOrNull(),
		...
		void Client::PlayerKilledPlayer(spades::client::Player &killer,
			...
			if (cg_killDistAnalyze) {
				if(killer.IsAlive() && killer.IsToolWeapon()){
					char buff[16];
					float dist;
					...
					if (!cg_distCalculation) {
						...
						sprintf(buff, "[%.02f 2D] ", dist);
						s += ChatWindow::ColoredMessage(buff, dist > 144 ? MsgColorRed : MsgColorTeam3);
					}
				}
			}
		...
		void Client::LocalPlayerPulledGrenadePin() {
			...
			if (!IsMuted()) {
				...
				AudioParam param;
				param.volume = cg_nadePinSoundGain;
				audioDevice->PlayLocal(c.GetPointerOrNull(), MakeVector3(.4f, -.3f, .5f), param);
			}
		}
