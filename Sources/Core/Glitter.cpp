...
namespace spades {
	...
	void Glitter::GlitterMap(const std::string &fileName) {
		...
		std::unique_ptr<spades::IStream> stream{FileManager::OpenForReading(fileName.c_str())};
		spades::client::GameMap *ret = spades::client::GameMap::Load(stream.get());
		if (DoGlitter(ret) < 0) {
			SPLog("Glitter failed. ");
			return;
		}
		...
		std::unique_ptr<spades::IStream> newStream = FileManager::OpenForWriting(newName.c_str());
		ret->Save(newStream.get());
		SPLog("Glitter done");
	}

	int Glitter::DoGlitter(spades::client::GameMap *map) {
		SPADES_MARK_FUNCTION();
		if (glitArgs.size() != 26) {
