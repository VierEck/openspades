...
namespace spades {
	...
	class StartupScreenGenericTab : spades::ui::UIElement, LabelAddable {
		...
		StartupScreenGenericTab(StartupScreenUI@ ui, Vector2 size) {
			...
			{
				ConfigItem cg_performanceSetting("cg_performanceSetting");
				string caption = cg_performanceSetting.IntValue == 0
					? "Enable Performance Setting"
					: "Disable Performance Setting";
				spades::ui::Button button(Manager);
				button.Caption = _Tr("StartupScreen", caption);
				button.Bounds = AABB2(160.0F, 30.0F, 350.0F, 30.0F);
				@button.Activated = spades::ui::EventHandler(this.OnPerformanceSettingsPressed);
				AddChild(button);
			}
			{
				spades::ui::Button button(Manager);
			...
			}
			{
				...
				button.Bounds = AABB2(160.0F, 99.0F, 350.0F, 30.0F);
		...
		private void OnPerformanceSettingsPressed(spades::ui::UIElement@ sender) {
			ConfigItem cg_performanceSetting("cg_performanceSetting");
			cg_performanceSetting.IntValue = cg_performanceSetting.IntValue == 0 ? 1 : 0;
