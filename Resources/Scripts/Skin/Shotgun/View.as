...
namespace spades {
	class ViewShotgunSkin : BasicViewWeapon {
		...
		// Attachment Points
		private Vector3 pumpAttachment = Vector3(0.125F, 0.0F, 0.5F);
		...
		private float pumpScale = 0.25F;
		private float rearSightScale = 0.05F;
		private float frontSightScale = 0.075F;
		...
		Vector3 MuzzlePosition { 
			get {
				return eyeMatrix * GetViewWeaponMatrix() * (Vector3(0.0F, 0.4F, -0.1F) + ManualViewWeaponOffset); 
			} 
		}
		Vector3 CaseEjectPosition { 
			get { 
				return eyeMatrix * GetViewWeaponMatrix() * (Vector3(0.0F, -0.1F, -0.1F) + ManualViewWeaponOffset); 
			} 
		}
		...
		void AddToScene() {
			...
			param.matrix = weapMatrix
				* CreateScaleMatrix(0.25F)
				* CreateTranslateMatrix(0.68F, 0.0F, 3.6F);
